...
class ProductRepositoryTestCase(TestCase):
    ...
    def test_create_product_for_ProductRepository_is_success(self):
        product = ProductRepository.create_instance(self.product_data)
        self.assertIsInstance(product, Product)

    def test_verify_is_data_is_correct_in_product_for_ProductRepository(self):
        product = ProductRepository.create_instance(self.product_data)
    ...
    def test_list_all_products_for_ProductRepository_is_success(self):
        products = ProductRepository.get_all_instances()
        self.assertEqual(products.count(), 1)

    def test_get_product_by_id_for_ProductRepository_is_success(self):
        product = ProductRepository.get_instance_by_id(self.product.id)
        self.assertEqual(product, self.product)

    def test_update_product_for_ProductRepository_is_success(self):
        ProductRepository.update_instance(
            self.product.id, {'name': 'novo nome'})
        product = ProductRepository.get_instance_by_id(self.product.id)
        self.assertEqual(product.name, 'novo nome')

    def test_delete_product_for_ProductRepository_is_success(self):
        ProductRepository.delete_instance(self.product.id)
        products = ProductRepository.get_all_instances()
